...
const NavItem: React.FC<{
    ...
    return (
        <button
            ...
            className={`flex items-center w-full px-4 py-3 text-sm font-black uppercase italic tracking-wider rounded-lg transition-colors duration-200 relative group ${
                isActive
                    ? 'bg-gradient-to-r from-orange-500/10 to-purple-500/10 text-white shadow-[inset_0_0_20px_rgba(249,115,22,0.05)]'
...
const ExternalNavItem: React.FC<{
    ...
    <a
        ...
        className="flex items-center w-full px-4 py-3 text-sm font-black uppercase italic tracking-wider rounded-lg text-gray-400 hover:bg-gray-800 hover:text-white transition-colors duration-200 group"
    >
        <div className="w-5 h-5 mr-3">{icon}</div>
        <span>{label}</span>
        <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 ml-auto text-gray-500 group-hover:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
    </a>
);
