...
const NavItem: React.FC<{
    ...
    label: string;
    subtext?: string;
}> = ({ page, activePage, setActivePage, icon, label, subtext }) => {
    const isActive = activePage === page;
    return (
        <button
            onClick={() => setActivePage(page)}
            className={`flex flex-col w-full px-4 py-3 rounded-xl transition-all duration-300 relative group mb-1.5 ${
                isActive
        ...
        >
            ...
            <div className="flex items-center w-full">
                ...
                <span className="truncate text-sm font-bold uppercase tracking-widest">{label}</span>
            </div>
            {subtext && (
                <span className="ml-8 text-[9px] font-black text-orange-500/60 uppercase tracking-tighter mt-1 group-hover:text-orange-400">
                    {subtext}
                </span>
            )}
        </button>
...
const ExternalNavItem: React.FC<{
    ...
    <a
        ...
        className="flex items-center w-full px-4 py-3 text-sm font-bold uppercase tracking-widest rounded-xl text-gray-500 hover:bg-white/5 hover:text-white transition-all duration-300 group mb-1.5"
    >
    ...
    const navLinks = (
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
            <div className="px-4 py-2 mb-6">
                <p className="text-[11px] font-black text-gray-600 uppercase tracking-widest italic mb-2 opacity-60">{t.dash_nav_menu}</p>
