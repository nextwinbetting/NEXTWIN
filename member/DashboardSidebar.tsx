...
const ExternalNavItem: React.FC<{
    ...
    >
        <div className="w-6 h-6 mr-3">{icon}</div>
        <span>{label}</span>
        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 ml-auto text-gray-500 group-hover:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
    </a>
);
            ...
            <nav className="flex-1 p-4 space-y-2">
                {navItems.map(item => {
                    if (item.type === 'external') {
                        return <ExternalNavItem key={item.id} href={item.href} icon={item.icon} label={item.label} />;
                    }
                    return (
                        <NavItem 
                            key={item.page}
