...
const InfoCard: React.FC<{
    ...
    <div 
        onClick={onClick}
        className={`group relative overflow-hidden rounded-2xl p-6 sm:p-8 cursor-pointer transition-all duration-500 hover:scale-[1.01] border ${
            featured 
    ...
    >
        <div className="flex items-center mb-5 relative z-10">
            <div className={`flex-shrink-0 h-10 w-10 flex items-center justify-center rounded-xl bg-gray-900 border border-white/10 text-orange-500 group-hover:bg-gradient-brand group-hover:text-white transition-all duration-500 shadow-lg`}>
                {icon}
            </div>
            <h2 className="ml-3 text-base font-bold text-white uppercase tracking-tight group-hover:text-orange-500 transition-colors">{title}</h2>
        </div>
        
        <p className="text-gray-500 text-[9px] font-bold leading-relaxed uppercase tracking-wider mb-6 group-hover:text-gray-300 transition-colors">
            {description}
        </p>

        <div className="flex items-center gap-2 relative z-10">
            <span className="text-[8px] font-black uppercase italic tracking-widest text-orange-500 group-hover:translate-x-1 transition-transform duration-500">
                {cta}
            </span>
            <div className="h-[1px] w-4 bg-gradient-brand rounded-full group-hover:w-8 transition-all duration-500"></div>
        </div>

        <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-brand opacity-[0.01] blur-3xl rounded-full"></div>
    </div>
);
        ...
        <div className="animate-fade-in max-w-5xl mx-auto pb-16 px-4">
            <div className="mb-10">
                <div className="inline-block bg-orange-500/5 border border-orange-500/10 px-3 py-1 rounded-full mb-4">
                    <span className="text-[8px] font-black text-orange-500 uppercase tracking-widest italic">ESPACE MEMBRE</span>
                </div>
                <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-white tracking-tight leading-tight truncate">
                    {t.dash_home_welcome} <span className="text-transparent bg-clip-text bg-gradient-brand">{username}</span>
                </h1>
                <p className="mt-2 text-[9px] font-bold text-gray-600 uppercase tracking-[0.4em] italic">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <InfoCard
                    ...
                    icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>}
