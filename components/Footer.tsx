...
const FooterLink: React.FC<{ onClick: () => void; children: React.ReactNode }> = ({ onClick, children }) => (
    <button onClick={onClick} className="text-sm font-bold text-white hover:text-brand-orange transition-all text-left italic uppercase tracking-widest hover:translate-x-2">
        {children}
    </button>
);
    ...
    <footer className="bg-brand-surface pt-40 pb-20 border-t border-white/5 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-20 mb-32">
          ...
          <div className="flex flex-col gap-10">
            <NextWinLogo />
            <p className="text-sm text-white font-bold leading-relaxed max-w-xs italic uppercase tracking-widest">
                L'architecture prédictive de référence mondiale. Gérée par la data, validée par l'intelligence artificielle.
            </p>
            <div className="flex gap-4">
                {[1, 2, 3].map(i => (
                    <div key={i} className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white hover:text-brand-orange hover:border-brand-orange transition-all cursor-pointer">
        ...
        <div className="pt-20 border-t border-white/5 flex flex-col lg:grid lg:grid-cols-3 items-center gap-12 text-center">
            <div className="flex flex-col gap-3 text-left">
                <p className="text-[10px] text-white font-black uppercase tracking-[0.5em] italic">© 2025 NEXTWIN ENGINE TECHNOLOGY</p>
                <div className="flex gap-6 text-[9px] text-white font-black uppercase italic tracking-[0.3em]">
            ...
            <div className="flex justify-center">
                <img src="https://stripe.com/img/v3/home/social.png" alt="Stripe Secure" className="h-8 opacity-60 invert" />
            </div>

            <p className="text-[9px] text-white font-bold leading-relaxed uppercase italic tracking-widest text-right">
