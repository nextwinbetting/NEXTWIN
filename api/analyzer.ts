...
export default async function handler(request: Request) {
    ...
    try {
        ...
        if (!sport || !team1 || !team2 || !betType) {
            return new Response(JSON.stringify({ error: "Paramètres manquants dans la requête." }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
        ...
        return new Response(JSON.stringify({ error: "Échec de l'analyse par NEXTWIN Engine.", details: errorMessage }), {
            status: 500,
            headers: { 'Content-Type': 'application/json' },
        });
    }
